...
Route::middleware(['auth'])->group(function () {
  ...
  //ruta para Servicio
  Route::post('agregar-servicio', [ServicioController::class, 'store'])->name('agregarDelito');
  ...
  // Ruta para mostrar la pantalla de bloqueo
  Route::get('/lockscreen', function () {
    session(['locked' => true]); // Marcamos que la sesión está bloqueada
    return view('lockscreen');
  })->name('lockscreen');

  // Ruta para desbloquear la sesión
  Route::post('/unlock', function (Request $request) {
    if (Auth::check() && Hash::check($request->password, Auth::user()->password)) {
      session(['locked' => false]); // Quitamos el bloqueo
      return redirect()->route('principal'); // Redirigir al dashboard o a la página deseada
    }
